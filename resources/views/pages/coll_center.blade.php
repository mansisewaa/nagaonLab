								...
								@foreach ($collection as $key => $collect)
									<tr>
										...
										<td style="display: flex;">
											<a href="{{ url('masters/edit-coll-center/' . $collect->id) }}" class="btn btn-outline-success btn-sm"
											style="margin-right:.6rem;"	data-toggle="modal" data-target="#editmodal{{ $collect->id }}" data-id="{{ $collect->id }}">Edit</a>
											{{-- Edit Modal --}}
												...
												<div class="modal-dialog modal-lg">
													...
													<div class="modal-content">
														...
														<div class="modal-body">
															<form action="{{ url('masters/updated-coll-center/' . $collect->id) }}" method="POST">
																{{ csrf_field() }}
																{{ method_field('PUT') }}
																...
																<div class="container-fluid" style="padding:inherit;">
																	<div class="form-row">
																		<div class="form-group col-md-6">
																			...
																			<input type="text" class="form-control" name="code" value="{{ $collect->code }}">
																		</div>
																		<div class="form-group col-md-6">
																			<label for="inputPassword4">Edit Name</label>
																			<input type="text" class="form-control" name="name" value="{{ $collect->name }}">
																		</div>
																	</div>
																	<div class="form-group">
																		<label> Edit Address</label>
																		<input type="text" class="form-control" name="address" value="{{ $collect->address }}">
																	</div>

																	<div class="form-row">
																		<div class="form-group col-md-6">
																			<label>Edit City</label>
																			<input type="text" class="form-control" name="city" value="{{ $collect->city }}">
																		</div>
																		<div class="form-group col-md-4">
																			<label> Edit State</label>
																			<input type="text" class="form-control" name="state" value="{{ $collect->state }}">
																		</div>
																		<div class="form-group col-md-2">
																			<label> Edit Zip</label>
																			<input type="text" class="form-control" name="zip" value="{{ $collect->zip }}">
																		</div>
																	</div>
																	<div class="form-row">
																		<div class="form-group col-md-6">
																			<label>Edit Mobile</label>
																			<input type="text" class="form-control" name="mobile" value="{{ $collect->mobile }}">
																		</div>
																		<div class="form-group col-md-6">
																			<label>Edit Email</label>
																			<input type="text" class="form-control" name="email" value="{{ $collect->email }}">
																		</div>
